...
{
    public class InterfaceConfViewModel
    {
        #region 属性
        /// <summary>
        /// 当前table页显示数据
        /// </summary>
        public List<InterfaceConfStru> dataInfo = new List<InterfaceConfStru>();
        /// <summary>
        /// Url参数列表
        /// </summary>
        ...
        /// <returns></returns>
        public List<InterfaceConfStru> GetDataInfo(CommConf options, int pageClick, ref Dictionary<string, string> sqlDic)
        {
            //查询数据总数
            List<InterfaceConfStru> tempDataInfo = new List<InterfaceConfStru>();
            CommonMethod comm = new CommonMethod();
            string errorMsg = string.Empty;
            string sql = @"SELECT * FROM InterfaceConf";

            DbConnStru SqlitePath = options.DbConnList.FirstOrDefault(o => o.key == "SqliteConn_1");

            var list = comm.Select<InterfaceConfStru>(sql, SqlitePath);
            if (!string.IsNullOrEmpty(errorMsg))
            {
                return null;
            }

            foreach (InterfaceConfStru item in list)
            {
            ...
            dataInfo = comm.TablePage<InterfaceConfStru>(tempDataInfo, ref sqlDic);
            return dataInfo;
        }
        #endregion
    }
