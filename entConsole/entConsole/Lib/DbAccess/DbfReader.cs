...
namespace entConsole.Lib.DbAccess
{
    ...
    public class DbfReader : IDisposable
    {
        ...
        private bool _IsReadDel = false;

        public DbfReader(string fileName, bool IsReadDel)
        {
            this._dbfFileName = fileName.Trim();
            this._IsReadDel = IsReadDel;
            try
            {
                this.OpenDbfFile();
            }
        ...
        public DbfReader(string fileName, string encodingName, bool IsReadDel)
        {
            this._dbfFileName = fileName.Trim();
        ...
        public void GetFileStream()
        {
            try
            {
                this._fileStream = File.Open(this._dbfFileName, FileMode.Open, FileAccess.Read, FileShare.ReadWrite);
        ...
        private int GetFieldCount()
        {
            ...
            int fCount = (this._dbfHeader.HeaderLength - DbfHeader.HeaderSize - 1) / DbfField.FieldSize;

            for (int k = 0; k < fCount; k++)
            {
                _fileStream.Seek(DbfHeader.HeaderSize + k * DbfField.FieldSize, SeekOrigin.Begin);  // 定位到每个字段结构区，获取第一个字节的值
        ...
        {
            _dbfFields = new DbfField[_fieldCount];

            try
            {
                _fileStream.Seek(DbfHeader.HeaderSize, SeekOrigin.Begin);
                for (int k = 0; k < _fieldCount; k++)
                {
                    this._dbfFields[k] = new DbfField();
        ...
        private DateTime ToDate(byte[] buf)
        {
            ...
            if (!int.TryParse(dateStr.Substring(0, 4), out year))
            {
                year = 1899;
            }

            if (!int.TryParse(dateStr.Substring(4, 2), out month))
            {
                month = 12;
            }

            if (!int.TryParse(dateStr.Substring(6, 2), out day))
            {
                day = 30;
            }
