...
{
    public class DbTableConfigController : Controller
    {
        private readonly CommConf _appConf;
        public DbTableConfigController(IOptions<CommConf> options)        {            _appConf = options.Value;        }
        ...
        public IActionResult Index(string id, string pageclick, string RecordTime, string userName, string Level)
        {
            ...
            DbTableConfigViewModel dataInfo = new DbTableConfigViewModel();
            if (string.IsNullOrEmpty(pageclick))
            {
                pageclick = "1";
            }
            var resultModel = new DbTableConfigViewModel
            {
