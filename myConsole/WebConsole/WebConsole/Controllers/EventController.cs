    ...
    public class EventController : Controller
    {
        ...
        public string DrawVol(string stockcode, string stockname)
        {
            ...
            for (int i = 0; i < list_vol.Count; i++)
            {
                ...
                li.Add(Convert.ToInt32(list_time[i].Remove(2,1)));
                li.Add(list_vol[i]);
                ll.Add(li);
            }
            ...
            string jsonData = JsonConvert.SerializeObject(ll.ToArray());
            return jsonData;
        }

        public string DrawVol_s(string stockcode, string stockname)
        {
            stockcode = "sz000001";
            string stockLink = string.Format("http://data.gtimg.cn/flashdata/hushen/minute/{0}.js?maxage=111", stockcode);
            HttpsGet httpget = new HttpsGet();
